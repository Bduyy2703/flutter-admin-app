...
import 'package:apehome_admin/screens/edit_care_service_screen.dart'; // Thêm import
...
class _CareServicesScreenState extends State<CareServicesScreen> {
  ...
  Future<void> _fetchServices() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
          'Lỗi khi tải danh sách dịch vụ: ${response.statusCode}',
        );
      }
    } catch (e) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Không thể tải danh sách dịch vụ. Vui lòng thử lại!'),
        ),
      );
    } finally {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 204) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Xóa dịch vụ thành công')));
      } else if (response.statusCode == 401) {
        print('Không có quyền - Token có thể không hợp lệ hoặc hết hạn');
        Navigator.pushReplacementNamed(context, '/login');
      } else {
  ...
  void _navigateToCreateService() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => CreateCareServiceScreen(shopId: widget.shopId),
      ),
    ).then((_) => _fetchServices());
  }

  void _navigateToEditService(Map<String, dynamic> service) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => EditCareServiceScreen(service: service),
      ),
    ).then((_) => _fetchServices());
  }
